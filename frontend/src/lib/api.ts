...
export const api = {
  login: (email: string, password: string) =>
    request<{ access_token: string; expires_in_minutes: number }>(
      "/auth/login",
      {
        method: "POST",
  ...
  executeAgent: (agent: string, task: string) =>
    request<{ agent: string; output: string; duration_ms: number }>(
      "/agents/execute",
      {
        method: "POST",
  ...
  uploadDocument: (file: File, productArea: string, version?: string) => {
    ...
    return request<DocumentRec>(`/documents/upload?${q}`, {
      method: "POST",
      body: fd,
    });
  },
  ...
  runWorkflow: (workflow: string, ticketId: string) =>
    request<RunResponse>("/workflows/run", {
      method: "POST",
      body: JSON.stringify({ workflow, ticket_id: ticketId }),
    }),
  workflowStatus: (runId: string) =>
    request<WorkflowStatus>(`/workflows/${runId}/status`),
